...
class OrderRepository {
  ...
  Future<Order> createCustomerOrder({
    ...
    print('📦 [createCustomerOrder] START');
    print('📦 customerId: $customerId');
    print('📦 items count: ${items.length}');
    print('📦 orderType: ${orderType.value}');
    ...
    print('📦 Order doc created: ${orderRef.id}');

    // Create order items as subcollection
    print('📦 Creating order items subcollection...');
    final orderItems = <OrderItem>[];
    for (var i = 0; i < items.length; i++) {
      ...
      if (menuItemId == null || menuItemId.isEmpty) {
        throw Exception('Invalid menu item: menuItemId is required');
      }

      final quantity = item['quantity'] as int?;
      print('📦   quantity: $quantity');
      if (quantity == null || quantity <= 0) {
        throw Exception('Invalid quantity for menu item $menuItemId');
      }

      final unitPrice = item['unitPrice'] as num?;
      print('📦   unitPrice: $unitPrice');
      if (unitPrice == null) {
        throw Exception('Invalid unit price for menu item $menuItemId');
      }

      // Fetch menu item name
      print('📦   Fetching menu item doc: $menuItemId');
      final menuItemDoc = await _firestore
          .collection('menu_items')
          .doc(menuItemId)
          .get();
      print('📦   Menu item exists: ${menuItemDoc.exists}');
      print('📦   Menu item name: ${menuItemDoc.data()?['name']}');

      print('📦   Adding item to subcollection...');
      final itemRef = await orderRef.collection('items').add({
      ...
      print('📦   Item doc created: ${itemRef.id}');
    ...
    print('📦 [createCustomerOrder] SUCCESS - Order ${order.id}');
    return order;
  }
