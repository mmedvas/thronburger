...
class _LiveQueueScreenState extends State<LiveQueueScreen> {
  ...
  void _showSystemNotification(Order order) {
    if (html.Notification.permission == 'granted') {
      html.Notification(
        'New Order #${order.orderNumber}',
        body: '${order.items.length} items - ${order.totalAmount} IQD',
        icon:
            'icons/Icon-192.png', // Make sure this icon exists or use a valid URL
      );
    }
  }
  ...
  void _subscribeToOrders() {
    ...
    _subscription = orderRepo.onNewOrder.listen((order) {
      if (_soundEnabled && order.isOnlineOrder) {
        _playNotificationSound();
        _showSystemNotification(order);
      }
